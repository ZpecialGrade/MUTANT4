...
public class DmitriyRestExceptionHandler {
	@ExceptionHandler(DmitriyApiException.class)
	public ProblemDetail handleApiException(DmitriyApiException ex, HttpServletRequest request) {
	...
	public ProblemDetail handleValidation(MethodArgumentNotValidException ex, HttpServletRequest request) {
		...
		return problem(HttpStatus.BAD_REQUEST, DmitriyErrorCode.VALIDATION_ERROR, "Validation failed", request.getRequestURI(), violations);
	}
	...
	public ProblemDetail handleNotReadable(HttpMessageNotReadableException ex, HttpServletRequest request) {
		return problem(HttpStatus.BAD_REQUEST, DmitriyErrorCode.BAD_REQUEST, "Malformed JSON request", request.getRequestURI(), null);
	}
	...
	public ProblemDetail handleErrorResponseException(ErrorResponseException ex, HttpServletRequest request) {
		...
		if (pd == null) {
			return problem(HttpStatus.valueOf(ex.getStatusCode().value()), DmitriyErrorCode.BAD_REQUEST, ex.getMessage(), request.getRequestURI(), null);
		}
	...
	public ProblemDetail handleNotFound(NoResourceFoundException ex, HttpServletRequest request) {
		return problem(HttpStatus.NOT_FOUND, DmitriyErrorCode.NOT_FOUND, "Not found", request.getRequestURI(), null);
	}

	@ExceptionHandler(Exception.class)
	public ProblemDetail handleAny(Exception ex, HttpServletRequest request) {
		return problem(HttpStatus.INTERNAL_SERVER_ERROR, DmitriyErrorCode.INTERNAL_ERROR, "Unexpected error", request.getRequestURI(), null);
	}
	...
	private ProblemDetail problem(HttpStatus status, DmitriyErrorCode errorCode, String message, String path, Object violations) {
